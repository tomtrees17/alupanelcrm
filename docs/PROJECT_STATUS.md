...
**界面**：发票详情页顶部红色横幅（谁、何时、什么原因）+ 作废按钮；列表里整行删除线 + 红色「已作废」标签 + 一个「已作废(n)」筛选页签（有作废发票时才出现）。**打印件和 Word 导出都加醒目的 `VOID / BATAL` 边框标记**——纸质单据绝不能被误当成有效发票（英文，遵守打印零中文规则）。
...
**安全/运维**：已完成——cookie 加固 / 强制改密 / 登录限速 / 审批职责分离 / **数据备份**(`tools/backup_db.php` 见 6e) / 修复财务逾期判定写死日期(`finance.php` 现用 `date('Y-m-d')`) / **响应式移动端布局 + PWA** / **仓库转 private + 服务器改走 SSH remote**(见 6f) / **服务器安全审计**(无入侵迹象，见 6f) / **审计日志**(见 6g) / **收款冲销**(见 6h) / **WhatsApp 通知**(见 6i) / **AI 智能查库存**(见 6j) / **控制器提示语全部 i18n 化**(55 条，原先印尼员工每天看到中文) / **自动化测试**(`tools/run_tests.php`，287 项)。
